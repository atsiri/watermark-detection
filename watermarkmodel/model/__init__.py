...
from watermarkmodel.utils import FP16Module
...
def get_watermarks_detection_model(name, device, fp16=True, pretrained=True, cache_dir='/tmp/watermarkmodel/model/models'):
    ...
    if pretrained:
        ...
        weights = torch.load(os.path.join(cache_dir, config['filename']), device)#map_location=torch.device('cpu') #device, 
...
MODELS = {
    'convnext-tiny': dict(
        ...
        repo_id='atsiri/convnext_watermark-detection', #'watermarkmodel/models',
        filename='convnext-tiny_watermarks.pth' #'convnext-tiny_watermarks_detector_datav1_batch8_lr01e-3_epoch5.pth'#'convnext-tiny_watermarks_detector.pth', #'convnext-notpretrained-t_3layer-head_inp256_datasetv1_10epochs.pth'#
    ),
